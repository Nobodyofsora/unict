...
describe("[EDIT] PUT: /tweets/:id", () => {
    ...
    it("Edit tweet", async () => {
        ...
        expect(result.body).to.have.property("tweet", String(currentTweet.tweet).split("").reverse().join(""));
    });
...
describe("[DELETE] DELETE: /tweets/:id", () => {
    ...
    it("Delete user", async () => {
        ...
        expectJson(result);
        expect(result.body).to.have.property("message", 'Tweet successfully deleted');
        expect(result).to.have.property("status", 200);
    });
